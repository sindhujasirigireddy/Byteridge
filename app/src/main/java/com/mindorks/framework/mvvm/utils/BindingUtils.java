...
public final class BindingUtils {
    ...
    @BindingAdapter({"adapter", "action", "attemptedTestResults"})
    public static void addQuestionItems(SwipePlaceHolderView mCardsContainerView, List<QuestionCardData> mQuestionList, int mAction, ObservableField<HashMap<Integer, Boolean>> attemptedQuestions) {
            ...
            if (mQuestionList != null) {
                ...
                for (QuestionCardData question : mQuestionList) {
                    if (question != null && question.options != null
                        && question.options.size() >= 3 && question.options.size()<=4
                    ) {
                        mCardsContainerView.addView(new QuestionCard(question, attemptedQuestions));
                    }
                }
